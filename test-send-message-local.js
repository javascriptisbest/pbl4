...
async function testSendMessageLocal() {
  ...
  try {
    ...
    console.log(
      `✅ Created sender: ${senderUser.fullName} (${senderUser._id})`
    );
    ...
    const receiverSignupResponse = await fetch(
    ...
    if (!receiverSignupResponse.ok) {
      throw new Error(
        `Receiver signup failed: ${receiverSignupResponse.status}`
      );
    }

    const receiverUser = await receiverSignupResponse.json();
    console.log(
      `✅ Created receiver: ${receiverUser.fullName} (${receiverUser._id})`
    );
    ...
    const sendResponse = await fetch(
      `${BACKEND_URL}/api/messages/send/${receiverUser._id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
