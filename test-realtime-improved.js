...
function createSocketConnection(user) {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on("connect", () => {
      ...
      console.log(
        `🔌 ${user.fullName} connected (${socket.id}) with userId: ${user._id}`
      );
      resolve(socket);
    });
    ...
    socket.on("connect_error", (error) => {
      ...
      reject(
        new Error(
          `Socket connection failed for ${user.fullName}: ${error.message}`
        )
      );
    });
  });
...
async function testMessageExchange(users, sockets) {
  ...
  const sendResponse = await fetch(
    `${BACKEND_URL}/api/messages/send/${user2._id}`,
    {
      method: "POST",
      headers: {
...
async function testIdNormalization(users) {
  ...
  const testCases = [
    ...
    {
      input: { _id: user2._id },
      expected: String(user2._id),
      description: "Object with _id",
    },
