...
export const connectDB = async () => {
  try {
    ...
    setInterval(() => {
      ...
      if (stats) {
        console.log(`📊 DB Pool: ${stats.collections} collections, ${stats.objects} objects`);
      }
    ...
    console.log(`Read preference: ${conn.connection.client.options.readPreference}`);
    ...
    if (connectionAttempts < MAX_RETRIES) {
      console.log(`🔄 Retrying connection in 5s... (${connectionAttempts}/${MAX_RETRIES})`);
      setTimeout(() => connectDB(), 5000);
    } else {
