...
class PerformanceMonitor {
  ...
  generateReport() {
    ...
    if (memory) {
      console.log(`💾 Memory: ${memory.used}MB / ${memory.total}MB (${Math.round(memory.used/memory.total*100)}%)`);
    }
    
    Object.entries(metrics).forEach(([name, metric]) => {
      console.log(`⏱️ ${name}: avg ${metric.avg.toFixed(2)}ms (${metric.count} calls, min: ${metric.min.toFixed(2)}ms, max: ${metric.max.toFixed(2)}ms)`);
    });
...
export const measureNetwork = (url, options) => performanceMonitor.measureNetworkRequest(url, options);
