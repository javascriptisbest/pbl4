  ...
  for (const test of tests) {
    ...
    try {
      ...
      if (response.status === test.expectStatus) {
        ...
        console.log(
          `   ❌ ${test.name} - Expected ${test.expectStatus}, got ${response.status}`
        );
      }
    } catch (error) {
      console.log(`   ❌ ${test.name} - ${error.message}`);
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      import("socket.io-client")
        ...
        .catch(() => {
          console.log(
            `   ⚠️ WebSocket test skipped (socket.io-client not available)`
          );
          resolve();
        });
    } catch (error) {
